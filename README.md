...
Для разработки проекта использовалась IDE `IntelliJ IDEA`. Для хостинга и контроля версий применялся `GitHub`. Проект был реализован на языке программирования
`Java`. Сборка осуществлялась с помощью `Gradle`. Тесты реализованы с помощью фреймворка модульного тестирования `Junit5`
При разработке UI тестов использовался фреймфорк `Selenide`, при разработке API-тесты использовался `REST-assured`, а при разработке мобильных тестов - `Appium`.
Для удаленного запуска UI тестов использовался `Selenoid`. Мобильные тесты применялись для тестирования устройств на удаленной ферме `Browserstack`, а также запускались
на эмуляторе в `Android Studio`. Произведена CI настройка в `Jenkins`. Результаты работы тестов логируются в `Allure Report`, `Allure TestOps` и `Telegram`. Также произведена интеграция с 
симстемой баг-трекинга `Jira`. 
